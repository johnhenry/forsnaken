...
export default function *({ apples=[], width=400, height=400, control={}, walls=[] }, ...snakes){
  while(true){
    if(control.over){
      break;
    }
    ...
    for (const [head, apple] of collideArray(snakes.map(({head})=>head), apples)){
      ...
      digesting.add(snake);
      yield [new CustomEvent('score', {detail:{subject:snake,score:apple.value},})];
    }
    ...
    for (const [head] of collideArray(snakes.map(({head})=>head), walls.map(({cells})=>cells).flat())){
    ...
    yield [new CustomEvent('draw', {detail:[...apples.filter(({alive})=>alive), ...snakes, ...walls]})];
  }
}